...
/**
 * NEW: Detect if a move leads to a drawish/repetitive position
 */
function isDrawishMove(move, fen, alternatives) {
    ...
    // Check for excessive simplification in non-winning positions
    if (alternatives.length > 0) {
        const topScore = alternatives[0].score;
        const isCapture = move.length === 5 || move.includes('x');
        
        // If position is roughly equal and we're capturing, might be simplifying
        if (isCapture && Math.abs(topScore) < 50 && gamePhase === "endgame") {
            const pieceCount = (fen.match(/[pnbrqkPNBRQK]/g) || []).length;
            
            // Avoid simplifying to drawn endgames
            if (pieceCount <= 12) {
...
/**
 * NEW: Detect aggressive, winning-attempt moves (AlphaZero style)
 */
function isAggressiveMove(move, fen, score) {
    ...
    // Piece to aggressive square (attacking position)
    const position = fen.split(' ')[0];
    const ranks = position.split('/');
    
    // Advanced pieces (ranks 4-7 for white perspective)
...
function applyAlphaZeroLogic(bestMove, alternatives) {
    ...
    // NEW: DRAW AVOIDANCE - Filter out drawish moves
    const viableMoves = alternatives.filter(alt => {
        ...
        return validateMoveForPosition(alt.move, currentFen);
    });
    
    // If all moves filtered, use unfiltered alternatives
    const candidateMoves = viableMoves.length > 0 ? viableMoves : alternatives;
    
    if (candidateMoves.length === 0) {
        debugLog("[ENGINE]", "⚠️ No candidate moves available");
        return bestMove;
    }
    
    // NEW: AGGRESSIVE MOVE PREFERENCE
    // In winning/advantageous positions, prefer aggressive continuations
    ...
    if (candidateMoves.length > 1) {
        ...
        // Enhanced unconventional selection with validation
        if (positionComplexity > 0.65 && scoreDiff < 45 && Math.random() < effectiveUnconventionalRate) {
            if (validateMoveForPosition(candidateMoves[1].move, currentFen)) {
        ...
        if (candidateMoves.length > 2 && positionComplexity > 0.72 && scoreDiff2 < 60) {
            if (Math.random() < (effectiveUnconventionalRate * 0.6)) {
                if (validateMoveForPosition(candidateMoves[2].move, currentFen) && 
                    isElegantMove(candidateMoves[2].move, candidateMoves, positionComplexity)) {
